...
public class PlayerBattleInfoDao extends RedisDao<PlayerBattleInfo> {
    public static final PlayerBattleInfoDao INSTANCE = new PlayerBattleInfoDao();
    ...
    public String getPrefix(Channel channel) {
        return num2Byt(RedisTopicConstant.PLAYER_BATTLE_INFO.getValue()) + ":"
                + num2Byt((Integer) channel.attr(AttributeKey.valueOf("userId")).get()) + ":";
    }
    ...
    @Override
    public PlayerBattleInfo getData(Channel ctx) {
        PlayerBattleInfo playerBattleInfo = new PlayerBattleInfo();

        playerBattleInfo.setId((Integer) get(ctx, "id"));
        playerBattleInfo.setBattleFieldId((Integer) get(ctx, "battleFieldId"));
        playerBattleInfo.setPosition((Object[]) get(ctx, "position")); // Adjust to specific type if known
        playerBattleInfo.setStatus((Byte) get(ctx, "status"));
        playerBattleInfo.setHp((Byte) get(ctx, "hp"));
        playerBattleInfo.setMoney((Integer) get(ctx, "money"));
        playerBattleInfo.setMainWeaponId((Integer) get(ctx, "mainWeaponId"));
        playerBattleInfo.setMainWeaponClipNum((Short) get(ctx, "mainWeaponClipNum"));
        playerBattleInfo.setMainWeaponAmmoNum((Short) get(ctx, "mainWeaponAmmoNum"));
        playerBattleInfo.setOffWeaponId((Integer) get(ctx, "offWeaponId"));
        playerBattleInfo.setOffWeaponClipNum((Short) get(ctx, "offWeaponClipNum"));
        playerBattleInfo.setOffWeaponAmmoNum((Short) get(ctx, "offWeaponAmmoNum"));
        playerBattleInfo.setCombatWeaponId((Integer) get(ctx, "combatWeaponId"));
        playerBattleInfo.setKillNum((Short) get(ctx, "killNum"));
        playerBattleInfo.setDeathNum((Short) get(ctx, "deathNum"));
        playerBattleInfo.setOccupyNum((Short) get(ctx, "occupyNum"));
        playerBattleInfo.setParty((Byte) get(ctx, "party"));
        playerBattleInfo.setInborn((Boolean) get(ctx, "inborn"));
        playerBattleInfo.setInHardPoint((Byte) get(ctx, "inHardPoint"));
