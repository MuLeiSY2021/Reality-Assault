...
public class BasementLostNotificationAcceptService{
    ...
    public static void run() {
        for (int i = 0; i < ThreadPoolConfig.CORE_POOL_SIZE; i++) {
            pool.execute(new BasementLostNotificationAcceptJob());
        }
    }
    ...
    private static class BasementLostNotificationAcceptJob implements Runnable {

        private final Consumer<Integer, Packet> consumer;

        public BasementLostNotificationAcceptJob() {
            this.consumer = KafkaConsumerFactory.createConsumer(Config.APPID);
            consumer.subscribe(Collections.singleton(BasementLostNotification.TOPIC));
        }
            ...
            try {
                while (true) {
                    ...
                    for (ConsumerRecord<Integer, Packet> record : records) {
                        if (record.value() instanceof BasementLostNotification) {
                            //TODO:战局结算处理评分胜负的部分（暂无）
                        }
                    }
                }
            } catch (Exception e) {
